...
for MODEL_DIR in "${MODEL_LIST[@]}"; do
    for DATA_DIR in "${DATASET_LIST[@]}"; do
        ...
        if [[ "$DATA_DIR" == *"US"* ]]; then
            echo "当前路径包含 'US'"
            OUTPUT_DIR="../output_70b/$(basename $MODEL_DIR)_MedQA_US"
        elif [[ "$DATA_DIR" == *"Mainland"* ]]; then
            echo "当前路径包含 'Mainland'"
            OUTPUT_DIR="../output_70b/$(basename $MODEL_DIR)_MedQA_Mainland"
        elif [[ "$DATA_DIR" == *"medmcqa"* ]]; then
            echo "当前路径包含 'MedMCQA'"
            OUTPUT_DIR="../output_70b/$(basename $MODEL_DIR)_MedMCQA"
        elif [[ "$DATA_DIR" == *"pubmedqa"* ]]; then
            echo "当前路径包含 'PubMedQA'"
            OUTPUT_DIR="../output_70b/$(basename $MODEL_DIR)_PubMedQA"
        else
